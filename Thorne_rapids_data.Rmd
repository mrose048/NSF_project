...
```{r franklin working directory working directory}
wd <- list()
...
all_rapids_sp <- vroom::vroom(paste0(wd$data, "Thorne_plots/cleaned_rapid_study_species.gz"))
filt <- names(all_rapids_sp)[1:6]
pres_col <- names(all_rapids_sp)[-c(1:6)]
...
for( i in 1:length(pres_col)){
  ...
  if(sp_symbol %in% plant_codes_accepted$symbol) {
    ...
    all_rapids_sp <- dplyr::rename(all_rapids_sp, pres_col2)
...
vroom::vroom_write(all_rapids_sp, 
...
vroom::vroom_write(rapids_sp, paste0(wd$data, "Thorne_plots/final_compiled_rapid_sp_codes.gz"))
```