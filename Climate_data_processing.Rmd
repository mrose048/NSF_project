...
cfp.area <- ggplot() +
  ...
  theme(legend.title = element_blank(),
        ...
        text = element_text(size = 5, family = "serif", color = 'black'),
  ...
  ggsn::scalebar(cfp.trans, dist = 250, dist_unit = "km", transform = FALSE, size = 5, family = 'serif')
cfp.area
...
# Soil data
These were downloaded from the gNATSGO database - includes pH, available water holding capacity, depth, and percent clay. These data were initially processed and resampled in ArcGIS using the toolbox and guide provided by gNATSGO.
...
# Theobald Landform data (Ecolgocially relevant ) (for the full CFP)
The classes include: "Peak/ridge (warm)", "Peak/ridge", "Peak/ridge (cool)", "Mountain/divide", "Cliff/steep slope", 
"Upper slope (warm)", "Upper slope", "Upper slope (cool)", "Upper slope (flat)", "Lower slope (warm)", "Lower slope",
...
# Geomorpho90m_v.1.0 Amatulli et al. 2020 - geomorphon classes
...
# Cropping environmental predictors to CFP extent
I use lists of rasters so that I can use lapply to crop them to the extent of the CFP
...
# Map of environmental predictors to be used for SDM (1981-2010 climate)
```{r}
plot(cfp81_10)
```

```{r write rasters california cfp}
